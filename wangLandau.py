...
# diffent models which are Wang-Landaugarithmable
from WLObjects import IsingGrid, PottsGrid
        ...
        while True:
            for iterations in range(0, grid.numSites):
                ...
                if(np.abs(enew) > grid.MAX_E):
                    ...
                    enew = energy - grid.neighborEnergy(randIndex)*2
                    continue
                gNew, gOld = logG[grid.energyIndex(
                    enew)], logG[grid.energyIndex(energy)]
                gRatioLog = gOld - gNew
            ...
            if(isFlatAverage(h) & ((count > minIterations) | (0 not in h))):
                ...
                for i in range(0, len(logG)):
                    # normalize logG vector to prevent overflows of exp()
                    logG[i] -= minG
                break
            count += 1
