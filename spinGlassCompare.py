...
def wangLandau(grid):
    ...
    #                    logG[i]-=minG       #normalize logG vector to prevent overflows of exp()
                    break
                if(verbose):
                    print("Iterations:" + str(fSweeps) + "\tSweeps:" +
                          str(count) + "\tNumber of energy states:" + str(hist.N))
        fFactor = np.sqrt(fFactor)
        fSweeps += 1
        print("F="+str(fFactor)+"\tSweeps:"+str(count) +
              "\tNumber of Energy States counted:"+str(hist.N))
    out = [0.0]*hist.N
...
MetropolisModule.runAlgorithm(
    grd, plotRange, thermalizationSweeps, measurementSweeps, 5, fileOut+"_Metropolis")
